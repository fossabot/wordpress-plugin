...
final class Reckon
{
    ...
    public function admin_notices()
    {
        ...
        if (in_array($current_screen->base, array('dashboard', 'plugins')) && !$this->api->is_connected()) {
            ...
            <div class="notice notice-success notice-large is-dismissible">
                <h3 class="notice-title"><?php _e('Awesome! Your new Reckon plugin is now active.', 'reckon'); ?></h3>
                <p><strong><?php _e('Take a few simple steps to complete your store setup.', 'reckon'); ?></strong></p>
                <p>
                    <a class="button button-primary" href="<?php echo admin_url('admin.php?page=' . $this->slug); ?>"><?php _e('Setup Reckon', 'reckon'); ?></a>
                </p>
            </div>
            <?php
        }
    ...
    {
        $defaults = array(
            'title'            => esc_attr__('Reckon Widget', 'reckon'),
            'link'             => '',
            'store_link'       => '',
            'type'             => '',
            'interact'         => 'modal',
            ...
            'auto_width'       => 'true',
            'fluid_width'      => 'false',
            'button_text'      => __('Add to cart', 'reckon'),
    ...
    public function get_store_block($store_url)
    {
        ...
        return '<!-- wp:' . $this->slug . '/store -->
            ...
            <noscript><a href="' . esc_url($store_url) . '" target="_blank" rel="noopener noreferrer">' . __('Shop now', 'reckon') . '</a></noscript>
            <!-- /wp:' . $this->slug . '/store -->';
    }
        ...
        return array_merge(
            ...
            array(
                array(
                    ...
                    'title' => __('Reckon Ecommerce', $this->lang),
                ),
            )
        );
    }
